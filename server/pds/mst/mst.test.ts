...
describe('MST Utilities', () => {
  ...
  describe('layerForKey', () => {
    it('computes consistent layers for same key', async () => {
    ...
    it('produces non-negative layers', async () => {
      const keys = [
        '2653ae71',
        'blue',
      ...
      for (const key of keys) {
        const layer = await layerForKey(key);
...
describe('MST Core Operations', () => {
  ...
  describe('add', () => {
    ...
    it('adds 500 records', async () => {
      const entries = await generateEntries(500);
      ...
      expect(await mst.count()).toBe(500);
    });
  });
...
describe('MSTNode Static Methods', () => {
  it('creates empty node', () => {
    const node = MSTNode.create(new MemoryBlockStore());
    expect(node.isEmpty()).toBe(true);
  });

  it('loads node from blockstore', async () => {
    const blockStore = new MemoryBlockStore();
    const node = MSTNode.create(blockStore);
    const cid = await randomCid();

    const updatedNode = await node.add('coll/key1', cid);
    const pointer = await updatedNode.getPointer();

    const loadedNode = await MSTNode.load(blockStore, pointer);
    expect(await loadedNode.has('coll/key1')).toBe(true);
  });

  it('throws when loading non-existent CID', async () => {
    const blockStore = new MemoryBlockStore();
    const fakeCid = await randomCid();

    await expect(MSTNode.load(blockStore, fakeCid)).rejects.toThrow('Block not found');
  });
});
