...
import { publishCanvas, saveCanvasToATProto, deleteCanvasFromATProto } from '../services/posts.js';
...
router.put('/api/canvases/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (agent) {
      ...
      if (!syncResult.success) {
        atprotoError = syncResult.error;
        console.error(`Failed to sync canvas to ATProto: ${syncResult.error}`);
      }
    } else {
      atprotoError = 'Unable to authenticate with ATProto';
      console.error('Failed to get authenticated agent for canvas sync');
    }
  } catch (error) {
    atprotoError = error instanceof Error ? error.message : 'Unknown error';
    console.error('Error syncing canvas to ATProto:', error);
  }

  // Re-fetch to get updated URI/rkey if they were set
  ...
  res.json({
    ...
    uri: finalCanvas.uri,
    rkey: finalCanvas.rkey,
    created_at: finalCanvas.created_at,
    updated_at: finalCanvas.updated_at,
    atproto_error: atprotoError // Include sync error if any (non-blocking)
  });
});
...
router.post('/canvases/:id/delete', async (req: Request, res: Response) => {
  ...
  // If canvas is synced to ATProto, delete from there too
  if (canvas.rkey) {
    try {
      const agent = await getAuthenticatedAgent(auth.session, auth.user);
      if (agent) {
        await deleteCanvasFromATProto(agent, auth.user, canvas.rkey);
      }
    } catch (error) {
